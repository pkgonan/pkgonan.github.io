...
## 목적
* HikariCP에는 Tomcat DBCP의 test-while-idle과 같은 유휴 상태의 커넥션을 갱신하는 기능이 없을까?
...
** 없다. test-while-idle처럼 특정 기간마다 반복적으로 커넥션을 갱신하는 방식이 아니다. 다른 방식으로 갱신한다.
* 그렇다면 Pool안에 있는 Connection 갱신은 어떻게?
** HikariCP는 내부적으로 아래와 같은 특정 타이밍에 Connection Validation Check를 수행 - HikariCP 구현체 분석
...
* HikariCP의 max-lifetime(Connection 생존 기간) 그리고 Database wait_timeout 설정의 상관관계
** HikariCP 공식 문서에서는 max-lifetime값을 Database wait_timeout 설정보다 최소 30초 이상 짧게 줄 것을 권고.
...
**** DB 요청이 지속적으로 온다면, Pool에서 커넥션을 가져오며 Connection.isValid()가 호출되며 커넥션 갱신한다.
...
**** 하지만, db요청이 없을 경우 wait_timeout에 의해 이미 커넥션은 종료되었기에 커넥션 재연결을 실시
...
** 둘째, 동시에 한번에 여러개의 커넥션을 지우고 생성하는게 아니라 시간 분포를 고르게하여 커넥션 생성 및 삭제로 인한 이슈를 방지.
